...
export const useEnquiereStatus = () => {
  const courseRef = collection(db, "status_enquiry");
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const colRef = collection(db, "status_enquiry");
  ...
  useEffect(() => {
    ...
    const getAllData = async () => {
      const docRef = doc(db, "status_enquiry", editId);
  ...
  const courseDelete = async (id) => {
    setLoading(true);
    await deleteDoc(doc(db, "status_enquiry", id));
    setDeleteId(id);
    setLoading(false);
    setAddCourse("");
  };
  ...
  const courseUpdate = async () => {
    setLoading(true);
    await updateDoc(doc(db, "status_enquiry", editId), {
      add_course: addCourse,
    });
    setAddCourse("");
    setLoading(false);
